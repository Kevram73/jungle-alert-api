    ...
    <form method="POST" action="{{ route('admin.products.update', $product) }}">
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div class="col-span-2">
                ...
                <input type="url" name="amazon_url" value="{{ old('amazon_url', $product->amazon_url) }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div class="col-span-2">
                <label class="block text-sm font-medium mb-2">Titre *</label>
                <input type="text" name="title" value="{{ old('title', $product->title) }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div class="col-span-2">
                <label class="block text-sm font-medium mb-2">Description</label>
                <textarea name="description" rows="4" class="w-full border rounded px-4 py-2">{{ old('description', $product->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" name="asin" value="{{ old('asin', $product->asin) }}" class="w-full border rounded px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Prix actuel (€) *</label>
                <input type="number" step="0.01" name="current_price" value="{{ old('current_price', $product->current_price) }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Prix cible (€)</label>
                <input type="number" step="0.01" name="target_price" value="{{ old('target_price', $product->target_price) }}" class="w-full border rounded px-4 py-2">
            </div>
