    ...
    <form method="POST" action="{{ route('admin.subscriptions.update', $subscription) }}">
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="plan" value="{{ old('plan', $subscription->plan) }}" required class="w-full border rounded px-4 py-2">
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="amount" value="{{ old('amount', $subscription->amount) }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Devise *</label>
                <input type="text" name="currency" value="{{ old('currency', $subscription->currency) }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Date de début *</label>
                <input type="datetime-local" name="starts_at" value="{{ old('starts_at', $subscription->starts_at->format('Y-m-d\TH:i')) }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Date de fin *</label>
                <input type="datetime-local" name="expires_at" value="{{ old('expires_at', $subscription->expires_at->format('Y-m-d\TH:i')) }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div class="col-span-2">
                <label class="block text-sm font-medium mb-2">Référence de paiement</label>
                <input type="text" name="payment_reference" value="{{ old('payment_reference', $subscription->payment_reference) }}" class="w-full border rounded px-4 py-2">
            </div>
        </div>
        ...
        <div class="mt-6 flex gap-4">
            ...
            <a href="{{ route('admin.subscriptions.index') }}" class="bg-gray-600 text-white px-6 py-2 rounded hover:bg-gray-700">
                Annuler
            </a>
        </div>
    </form>
