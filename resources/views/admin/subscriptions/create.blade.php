    ...
    <form method="POST" action="{{ route('admin.subscriptions.store') }}">
        ...
        <div class="grid grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="plan" value="{{ old('plan') }}" placeholder="premium_simple ou premium_deluxe" required class="w-full border rounded px-4 py-2">
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="amount" value="{{ old('amount') }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Devise *</label>
                <input type="text" name="currency" value="{{ old('currency', 'EUR') }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Date de début *</label>
                <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2">Date de fin *</label>
                <input type="datetime-local" name="expires_at" value="{{ old('expires_at') }}" required class="w-full border rounded px-4 py-2">
            </div>

            <div class="col-span-2">
                <label class="block text-sm font-medium mb-2">Référence de paiement</label>
                <input type="text" name="payment_reference" value="{{ old('payment_reference') }}" class="w-full border rounded px-4 py-2">
            </div>
        </div>
